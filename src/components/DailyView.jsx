...
import { computeSegments, resolveBlockTime, SEGMENT_IDS, SEGMENT_LABELS } from "../utils/anchorSegments";
import { generateAnchorBlocks } from "./AnchorBlocks";
...
export default function DailyView({
  ...
  const { times } = useAnchorTimes({ date, city: location?.city, country: location?.country, lat: location?.lat, lng: location?.lng });
  const segments = useMemo(() => computeSegments(times), [times]);
  const anchorBlocks = useMemo(() => generateAnchorBlocks(times), [times]);
  ...
  const { userBlocks, positionedAnchors } = useMemo(() => {
    const withDur = resolvedBlocks.map((b) => ({
      ...b,
      duration: b.duration || DEFAULT_DUR,
    }));
    const allBlocks = [...anchorBlocks, ...withDur];
    ...
    for (const b of laid) {
      ...
      if (b.isAnchor) anchors.push(positioned);
      else users.push(positioned);
    }
    return { userBlocks: users, positionedAnchors: anchors };
  }, [resolvedBlocks, anchorBlocks]);
