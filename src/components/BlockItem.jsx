...
export default function BlockItem({ block, top, height, col = 0, totalCols = 1, isNew, isAnchor, extraTypeColors = {}, onEdit, onDelete, onCopy, onTimeChange, onResize, onToggleTask }) {
  ...
  const textColor = isAnchor ? "#2d8a4e" : (colors.text === "#94a3b8" ? "#b0b0ba" : colors.text);
  const fw = (block.type === "anchor" || isAnchor) ? 600 : 400;
  ...
  const resetVisuals = useCallback(() => {
    ...
    el.style.zIndex = isAnchor ? "10" : String(Math.ceil(top) + 1);
    el.style.boxShadow = "none";
    el.style.opacity = "1";
    el.style.cursor = isAnchor ? "default" : "pointer";
  ...
  }, [top, height, leftPos, colWidth, startDisplay, endTime, isAnchor, dur]);
  ...
  const onPointerUp = (e) => {
    ...
    if (holdTimer.current) {
      ...
      if (e.pointerType === "touch" && !isAnchor) onEdit(block);
      return;
    }
    if (!drag.current) return;
  ...
  return (
    <>
      <div
        ref={elRef}
        className={`timeline-block ${isNew ? "new-block" : ""} ${isAnchor ? "anchor-block" : ""}`}
        style={{
          top,
          ...
          width: colWidth,
          right: "auto",
          background: isAnchor ? "rgba(34, 139, 34, 0.10)" : colors.bg,
          borderLeft: `3px solid ${isAnchor ? "#2d8a4e" : colors.border}`,
          zIndex: isAnchor ? 10 : Math.ceil(top) + 1,
